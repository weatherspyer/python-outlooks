...
def main():
    ...
    for loc in locations:
        for d in days_to_run:
            ...
            # 1. Insert structured base row down to Row 2 coordinates
            sheet.insert_row(row, 2, value_input_option="USER_ENTERED")
            
            # 2. Inject validation/archive targets into cells AL2 and AM2
            if VALID_TIME:
                sheet.update_acell("AL2", VALID_TIME)
            
            # Always update AM2 with our clean, standardized URL path
            sheet.update_acell("AM2", ARCHIVE_URL)

            print(f"Inserted {loc['name']} WPC Day {d} | AL2: {VALID_TIME} | AM2: {ARCHIVE_URL} | Batch ID: {BATCH_ID}")
