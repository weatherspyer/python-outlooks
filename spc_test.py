...
VALID_TIME = ""
TARGET_SHEET_NAME = ""  # Global tracking assignment for webhook execution
...
def main():
    ...
    spreadsheet = gc.open_by_key("1HSLnDqg243qkgVJb7tpsnKLEDiaLFM0cCLwU5LQndsg")
    ...
    for loc in locations:
        for d in days_to_run:
            ...
            # Determine appropriate dynamic archive URLs
            archive_url = ""
            if d == "1":
            ...
            # Build standard base row structure tracking columns A - AN (Indices 0 - 39)
            row = [
                timestamp,
                loc["name"],
                loc["wfo"],
                ...
                d,
                ISSUE,
                "", "", "", "",  # H, I, J, K formulas tracking
                loc.get("region", ""),
                "",              # M formula allocation
                "",              # N formula allocation
                ...
                "", "", "", "", 
                VALID_TIME if VALID_TIME else "",  # Column AL (Directly inserted)
                archive_url if archive_url else "", # Column AM (Directly inserted)
                "" 
            ]
